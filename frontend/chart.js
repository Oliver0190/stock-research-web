import { escape, money, direction } from './format.js';

export function chart(points = [], { compact = false, change = null } = {}) {
  const valid = points.filter(p => Number.isFinite(p.close));
  if (valid.length < 2) return compact ? '<span class="muted">—</span>' : '<div class="chart-empty">暂无走势数据</div>';
