...
export function sidebar(data, route) {
  return data.stocks.map(s => `<a class="stock-link ${route === s.symbol ? 'active' : ''}" href="${routeHref(marketInfo(data).id, 'stock', s.symbol)}" ${route === s.symbol ? 'aria-current="page"' : ''}><span class="stock-link-name">${e(s.name)}<small>${e(stockCode(s, marketInfo(data).id))}</small></span>${s.unread ? `<span class="unread-badge" aria-label="${s.unread} 条重要记录未查看">${s.unread}</span>` : ''}</a>`).join('');
}
...
export function watchTable(data, state) {
  ...
  return `<div class="table-scroll"><table class="stock-table"><thead><tr><th>股票</th><th class="numeric">最近价格</th><th class="numeric">当日涨跌</th><th class="trend-column">近 30 日</th><th>最近观察</th><th><span class="sr-only">打开档案</span></th></tr></thead><tbody>${list.map(s => {
    ...
    return `<tr><td><a class="stock-cell" href="${routeHref(marketInfo(data).id, 'stock', s.symbol)}"><span><strong>${e(s.name)}</strong><small>${e(stockCode(s, marketInfo(data).id))} ${s.unread ? '<span class="new-indicator">新</span>' : ''}</small></span></a></td><td class="numeric"><strong class="price">${money(a?.kline.close)}</strong><small>${p ? `${p.data_date}${p.complete ? '' : ' · 盘中'}` : '等待行情'}</small></td><td class="numeric"><span class="change-pill ${direction(a?.kline.pct_change)}">${pct(a?.kline.pct_change)}</span></td><td class="trend-column">${chart(p?.chart?.slice(-30), { compact: true })}</td><td class="observation-cell"><span>${e(s.summary || (a ? a.indicators.ma.arrangement.split('(')[0] : '等待首次更新'))}</span>${badge(s.status)}</td><td><a class="row-link" href="${routeHref(marketInfo(data).id, 'stock', s.symbol)}" aria-label="打开${e(s.name)}档案">→</a></td></tr>`;
  }).join('')}</tbody></table></div>`;
}

function closingDigest(data) {
  const closing = data.reports.filter(r => r.kind === 'closing');
  return `<section class="panel closing-panel"><div class="closing-content"><div class="closing-heading"><h2>收盘摘要</h2><span>${data.selected_date}</span></div>${closing.length ? `<p>${closing.length} / ${data.stocks.length} 只已归档。${closing.some(r => r.importance === 'important') ? '含重点变化。' : '无重点变化。'}</p><div class="digest-links">${closing.slice(0, 6).map(r => `<a href="${routeHref(marketInfo(data).id, 'stock', r.symbol)}">${e(data.stocks.find(s => s.symbol === r.symbol)?.name || r.symbol)} <span>→</span></a>`).join('')}</div>` : '<p>尚未归档。待完整收盘数据到齐后更新。</p>'}</div></section>`;
}

export function detail(data, stock, state) {
  const p = stock.snapshot, a = p?.analysis;
  const note = state.drafts[draftKey(marketInfo(data).id, stock.symbol)] ?? stock.profile.note;
  return `<div class="detail-heading"><div class="stock-title"><div><div class="eyebrow">个股档案 <span>·</span> ${e(stockCode(stock, marketInfo(data).id))}</div><h1>${e(stock.name)}</h1><div class="stock-title-meta">${badge(stock.status)}<span>${p ? `${p.complete ? '收盘' : '盘中参考'} · ${p.data_date}` : '等待行情'}</span></div></div></div><div class="heading-actions">${datePicker(data, state.day)}${refreshButton(data, stock.symbol)}</div></div>
